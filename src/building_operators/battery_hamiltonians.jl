...
function Z_model_hamiltonian(majorana_operators)
    ...
    hamiltonian_matrix = spzeros(2^(n_majorana/2), 2^(n_majorana/2))
    for i in 1:Int(floor(n_majorana/2)) 
        hamiltonian_matrix .+= -2 * im .* majorana_operators[2 * i - 1] * majorana_operators[2 * i] 
    end
...
function X_model_hamiltonian(majorana_operators)
    n_majorana = length(majorana_operators)
    hamiltonian_matrix = spzeros(2^(n_majorana/2), 2^(n_majorana/2))
    for k in 1: Int(floor(n_majorana/2)) 
        if k == 1
            hamiltonian_matrix .+= sqrt(2) .* majorana_operators[1]
        else
